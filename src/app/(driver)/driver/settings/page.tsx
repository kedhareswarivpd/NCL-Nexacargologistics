...
import type { DriverProfile, DeliveryProof } from "@/lib/types";
...
export default function DriverSettingsPage() {
  ...
  async function handleProofUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    const { data: uploaded, error } = await supabase.storage.from("delivery-proofs").upload(path, file);
    ...
    if (!error && uploaded) {
      ...
      setUploadDone(true);
      // Refresh proof list
      const { data } = await supabase.from("delivery_proofs").select("*").order("uploaded_at", { ascending: false }).limit(5);
  ...
  async function saveProfile() {
    if (!profile) return;
    await supabase.from("driver_profiles").update({ phone: profile.phone, location: profile.location }).eq("id", profile.id);
  }
