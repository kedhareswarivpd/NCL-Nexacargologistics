...
import type { DriverProfile, DeliveryStop } from "@/lib/types";
...
export default function DriverProfilePage() {
  ...
  const [recentDeliveries, setRecentDeliveries] = useState<DeliveryStop[]>([]);
  ...
  useEffect(() => {
    ...
    supabase
      .from("delivery_stops")
      .select("*, driver_routes!inner(date)")
      .eq("status", "done")
      .order("scheduled_time", { ascending: false })
      .limit(5)
      .then(({ data }) => data && setRecentDeliveries(data));
  }, [user]);
  ...
  const STATS = [
    { label: "Total Deliveries", value: profile ? profile.total_deliveries.toLocaleString() : "—", Icon: Package, color: "text-tertiary", bg: "bg-tertiary/10" },
    { label: "On-Time Rate", value: profile ? `${profile.on_time_rate}%` : "—", Icon: TrendingUp, color: "text-green-400", bg: "bg-green-400/10" },
    { label: "Avg Rating", value: profile ? `${profile.avg_rating} / 5` : "—", Icon: Star, color: "text-secondary", bg: "bg-secondary/10" },
    { label: "Hours This Week", value: profile ? `${profile.hours_this_week}h` : "—", Icon: Clock, color: "text-on-surface", bg: "bg-white/5" },
  ];
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <Card className="p-5 space-y-4">
            ...
            {[
              ...
              { label: "Customer Satisfaction", pct: profile ? Math.round(profile.avg_rating * 20) : 0 },
