...
import { sendShipmentStatusUpdate } from "@/lib/notifications";
...
export default function DeliveriesPage() {
  ...
  async function handleStatusUpdate(d: any, newStatus: string) {
    await supabase.from("deliveries").update({ status: newStatus }).eq("id", d.id);
    await sendShipmentStatusUpdate({
      tracking_id: d.delivery_code,
      status: newStatus,
      customer_name: d.driver,
      eta: d.eta,
    });
    setNotifySent(d.id);
    load();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setSaving(true);
    await supabase.from("deliveries").insert([{ ...form, progress: parseInt(form.progress), status: "Pending" }]);
