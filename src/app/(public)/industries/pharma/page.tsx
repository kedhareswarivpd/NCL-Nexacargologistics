...
import { ArrowLeft, CheckCircle2, Thermometer, ShieldCheck, FileText, Clock } from "lucide-react";
  ...
  return (
    <div className="min-h-screen bg-[#0B1F3A] text-white pt-24 pb-16">
      <div className="relative h-72 overflow-hidden">
        <img src="https://images.unsplash.com/photo-1584308666744-24d5c474f2ae?w=1200&q=80" alt="Pharma" className="w-full h-full object-cover opacity-40" />
        ...
        <div className="absolute bottom-8 left-1/2 -translate-x-1/2 text-center w-full px-6">
          ...
          <h1 className="text-4xl md:text-5xl font-bold mt-2">Pharma & Healthcare</h1>
        </div>
      </div>
      ...
      <div className="max-w-5xl mx-auto px-6 mt-10">
        ...
        <motion.div initial="hidden" animate="show" variants={stagger} className="space-y-10">
          <motion.p variants={fadeUp} className="text-lg text-blue-200/70 max-w-3xl">
            We keep pharmaceutical cargo at exactly the right temperature from pickup to delivery, with full regulatory documentation included. Our cold-chain network is built specifically for life-science shipments.
          </motion.p>

          <motion.div variants={stagger} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
            {[
              { icon: Thermometer, title: "Cold Chain",         desc: "2°C–8°C and -20°C controlled throughout transit." },
              { icon: ShieldCheck, title: "GDP Compliant",      desc: "Full compliance with Good Distribution Practice." },
              { icon: FileText,    title: "Documentation",      desc: "Temperature logs, CoAs and regulatory paperwork." },
              { icon: Clock,       title: "24/7 Monitoring",    desc: "Live sensor alerts at every point in the journey." },
          ...
          <motion.div variants={fadeUp} className="rounded-2xl border border-white/10 bg-white/[0.03] p-8">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {["Temperature-controlled air freight","Refrigerated road transport","Clinical trial logistics","Controlled substance handling","Regulatory documentation","Customs clearance for pharmaceuticals","Returns & destruction management","Serialisation & track-and-trace"].map(f => (
          ...
          <motion.div variants={fadeUp} className="rounded-2xl border border-[#00C2FF]/20 bg-[#00C2FF]/5 p-8 flex flex-col sm:flex-row items-center justify-between gap-6">
            <div>
              <h3 className="text-xl font-bold mb-1">Need a cold-chain solution?</h3>
              <p className="text-blue-200/60 text-sm">Speak with our pharma logistics specialists.</p>
            </div>
