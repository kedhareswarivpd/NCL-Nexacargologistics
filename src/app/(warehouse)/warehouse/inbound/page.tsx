...
export default function InboundPage() {
  ...
  useEffect(() => {
    supabase
      ...
      .then(({ data }) => data && setShipments(data));
  }, []);
  ...
  const arriving = shipments.filter((i) => i.status === "Arriving").length;
  const inProgress = shipments.filter((i) => ["Unloading", "Inspecting", "Putaway"].includes(i.status)).length;
  const completed = shipments.filter((i) => i.status === "Completed").length;
