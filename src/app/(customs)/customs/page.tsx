...
export default function CustomsDashboardPage() {
  ...
  useEffect(() => {
    async function load() {
      const { data } = await supabase.from("customs_clearance").select("*").order("created_at", { ascending: false }).limit(5);
      if (data) {
        setClearances(data);
        setStats({
          total: data.length,
          pending: data.filter((s) => s.status === "Pending Review").length,
          approved: data.filter((s) => s.status === "Approved").length,
          onHold: data.filter((s) => s.status === "On Hold").length,
        });
      }
      setLoading(false);
    }
