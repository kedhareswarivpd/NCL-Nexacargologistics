...
export default function WarehouseDashboard() {
  ...
  useEffect(() => {
    ...
    supabase.from("storage_zones").select("*").then(({ data }) => data && setZones(data));
    supabase.from("inbound_shipments").select("*").order("date", { ascending: false }).limit(4).then(({ data }) => data && setInbound(data));
    supabase.from("warehouse_tasks").select("*").eq("status", "Pending").limit(4).then(({ data }) => data && setTasks(data));
    supabase.from("warehouse_throughput").select("day,units").order("day", { ascending: true }).limit(7).then(({ data }) => data && setThroughput(data));
  }, []);
  ...
  const STATS = [
    { label: "Total SKUs", value: totalSKUs.toLocaleString(), Icon: Boxes },
    { label: "Inbound Today", value: String(inboundToday), Icon: PackageOpen },
    { label: "Outbound Today", value: String(outboundToday), Icon: Truck },
    { label: "Pending Tasks", value: String(pendingTasks), Icon: ClipboardList },
  ];
