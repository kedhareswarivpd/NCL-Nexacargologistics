...
type NotifRow = { id: string; subject?: string; message: string; status: string; created_at: string };
...
export function PortalHeader({ userRole = "Premium Member" }: { userRole?: string }) {
  ...
  useEffect(() => {
    if (!bellOpen) return;
    if (role === "admin") {
      supabase.from("notifications").select("id, subject, message, status, created_at")
        .order("created_at", { ascending: false }).limit(6)
        .then(({ data }) => { if (data) setNotifs(data as NotifRow[]); });
    } else if (role === "customer") {
      supabase.from("quotes").select("id, origin, destination, status, created_at")
        .order("created_at", { ascending: false }).limit(6)
        .then(({ data }) => { if (data) setQuotes(data as QuoteRow[]); });
    }
  }, [bellOpen, role]);
                    ...
                    notifs.length === 0
                      ...
                      : notifs.map(n => (
                          <div key={n.id} className="px-4 py-3 hover:bg-white/5 transition-colors">
                            <p className="text-sm text-on-surface truncate">{n.subject ?? n.message}</p>
