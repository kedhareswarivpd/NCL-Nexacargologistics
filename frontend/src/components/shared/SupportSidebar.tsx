...
import { LayoutDashboard, MessageSquare, Phone, Headphones, BookOpen, HelpCircle, LogOut, Users } from "lucide-react";
...
const navItems = [
  ...
  { icon: Headphones, label: "Live Chat", href: "/support/chat" },
  ...
  return (
    <aside className="fixed left-0 top-0 h-full flex flex-col p-4 bg-surface-container-low/20 backdrop-blur-md border-r border-white/5 shadow-xl w-[280px] z-50 hidden lg:flex">
      <div className="mb-12 px-4 mt-2">
        ...
        <p className="text-xs text-on-surface-variant opacity-70 uppercase tracking-widest mt-1">Support Portal</p>
      </div>
          ...
          return (
            <Link key={item.label} href={item.href}
              className={cn(
                ...
                isActive
                  ? "bg-white/5 text-on-tertiary-container shadow-[0_0_15px_rgba(0,128,202,0.2)] translate-x-1"
                  : "text-on-surface-variant hover:bg-white/5 hover:text-on-surface"
              )}>
              <item.icon className={cn("w-5 h-5", isActive ? "text-on-tertiary-container" : "text-on-surface-variant")} />
              {item.label}
            </Link>
          );
        })}
