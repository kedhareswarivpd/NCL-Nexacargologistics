...
export default function WarehouseDashboard() {
  ...
  useEffect(() => {
    ...
    // TODO: no backend source yet for storage zones or weekly throughput.  // NOSONAR
    setZones([]);
    setThroughput([]);
  }, []);
              ...
              return (
                <div key={b.id}>
                  ...
                  <div className="w-full h-1.5 bg-white/5 rounded-full overflow-hidden">
                    <div
                      className={`h-full rounded-full animate-progress-fill ${pct >= 90 ? "bg-error" : pct >= 70 ? "bg-on-tertiary-container" : "bg-tertiary"}`}  // NOSONAR
                      style={{ width: `${pct}%` }}
                    />
                  </div>
                </div>
