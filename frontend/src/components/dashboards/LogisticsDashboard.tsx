...
export default function LogisticsDashboard() {
  ...
  async function load() {
    setLoading(true);
    try {
      const all = await shipmentsApi.list();
      const data = (all ?? []).slice(0, 8);
      setShipments(data);
      setStats({
        total:     data.length,
        inTransit: data.filter((s: any) => s.status === "In Transit").length,
        delayed:   data.filter((s: any) => s.status === "Delayed").length,
        delivered: data.filter((s: any) => s.status === "Delivered").length,
      });
    } catch {
      setShipments([]);
      setStats({ total: 0, inTransit: 0, delayed: 0, delivered: 0 });
    }
    setLoading(false);
  }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const createdId = created?.tracking_id ?? form.tracking_id;
        ...
        {showModal && (
          ...
          >
            ...
            >
              ...
              <form noValidate onSubmit={handleSubmit} className="p-6 space-y-5 max-h-[75vh] overflow-y-auto">
