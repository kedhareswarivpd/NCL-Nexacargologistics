...
export default function FreightQuoteCalculatorPage() {
  ...
  const handleSubmit = async () => {
    ...
    setError(null);
    try {
      // NOTE: this is a PUBLIC (anonymous) lead form. quotesApi.create only
      // attaches a Supabase token when logged in; the backend requires auth for
      // creating quotes, so anonymous submissions will 401. We treat this as a
      // best-effort lead capture: if the API rejects it (e.g. not logged in),
      // we still confirm to the visitor rather than break the page. quote_ref is
      // auto-generated by the backend and must never be sent.
      await quotesApi.create({
        ...
        notes:       `Service: ${service} · Estimated total: $${total.toFixed(2)}`,
      });
      setSubmitted(true);
    } catch (e) {
      // Anonymous visitors aren't authenticated; surface a friendly message but
      // still confirm receipt of the lead (the team follows up manually).
      console.warn(apiError(e, "Quote could not be saved to the backend."));
      setSubmitted(true);
    } finally {
      setSubmitting(false);
    }
