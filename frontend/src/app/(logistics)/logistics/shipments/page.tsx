    ...
    <div className="p-6 space-y-6">
      <div className="flex items-start justify-between gap-4">
        ...
        <div className="flex gap-2 shrink-0">
          <button type="button" onClick={load} className="flex items-center gap-2 px-4 py-2.5 rounded-lg bg-white/5 text-on-surface-variant text-xs font-semibold hover:bg-white/10 transition-colors">
            <RefreshCw className="h-4 w-4" />
          </button>
          <button type="button" onClick={() => setShowForm(!showForm)} className="flex items-center gap-2 px-4 py-2.5 rounded-lg bg-tertiary/20 text-tertiary text-xs font-semibold hover:bg-tertiary/30 transition-colors">
        ...
        <Card className="p-5 space-y-4 border border-tertiary/20">
          ...
          <form noValidate onSubmit={handleAdd} className="space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div><label htmlFor="add-origin" className="text-xs uppercase tracking-widest text-on-surface-variant">Origin</label><input id="add-origin" value={form.origin} onChange={(e) => { setForm({ ...form, origin: e.target.value }); setShipmentErrors(p => ({...p, origin: ""})); }} placeholder="e.g. Shanghai" className={`${inputCls} ${shipmentErrors.origin ? "border-red-500" : ""}`} />{shipmentErrors.origin && <p className="text-xs text-error mt-1">{shipmentErrors.origin}</p>}</div>
              <div><label htmlFor="add-destination" className="text-xs uppercase tracking-widest text-on-surface-variant">Destination</label><input id="add-destination" value={form.destination} onChange={(e) => { setForm({ ...form, destination: e.target.value }); setShipmentErrors(p => ({...p, destination: ""})); }} placeholder="e.g. Rotterdam" className={`${inputCls} ${shipmentErrors.destination ? "border-red-500" : ""}`} />{shipmentErrors.destination && <p className="text-xs text-error mt-1">{shipmentErrors.destination}</p>}</div>
              <div><label htmlFor="add-cargo-type" className="text-xs uppercase tracking-widest text-on-surface-variant">Cargo Type</label><input id="add-cargo-type" value={form.cargo_type} onChange={(e) => setForm({ ...form, cargo_type: e.target.value })} placeholder="e.g. Electronics" className={inputCls} /></div>
              <div><label htmlFor="add-weight" className="text-xs uppercase tracking-widest text-on-surface-variant">Weight</label><input id="add-weight" value={form.weight} onChange={(e) => setForm({ ...form, weight: e.target.value })} placeholder="e.g. 5,000 kg" className={inputCls} /></div>
              <div><label htmlFor="add-customer" className="text-xs uppercase tracking-widest text-on-surface-variant">Customer</label><input id="add-customer" value={form.customer_name} onChange={(e) => setForm({ ...form, customer_name: e.target.value })} placeholder="e.g. Amazon EU" className={inputCls} /></div>
            </div>
                ...
                <tr key={s.id} className="hover:bg-white/5 transition-colors group">
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-all">
                      <button type="button" onClick={() => openEdit(s)} className="p-1.5 rounded-lg hover:bg-tertiary/10 text-tertiary transition-colors" title="Edit">
                        <Edit className="h-4 w-4" />
                      </button>
                      <button type="button" onClick={() => handleDelete(s.id)} className="p-1.5 rounded-lg hover:bg-error/10 text-error transition-colors" title="Delete">
          ...
          <div className="w-full max-w-lg rounded-2xl border border-white/10 bg-surface-container shadow-2xl">
            ...
            <form noValidate onSubmit={handleEditSave} className="p-6 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div><label htmlFor="edit-cargo-type" className="text-xs uppercase tracking-widest text-on-surface-variant">Cargo Type</label><input id="edit-cargo-type" value={editForm.cargo_type} onChange={e => setEditForm(p => ({...p, cargo_type: e.target.value}))} className={inputCls} /></div>
                <div><label htmlFor="edit-weight" className="text-xs uppercase tracking-widest text-on-surface-variant">Weight</label><input id="edit-weight" value={editForm.weight} onChange={e => setEditForm(p => ({...p, weight: e.target.value}))} className={inputCls} /></div>
                <div><label htmlFor="edit-customer" className="text-xs uppercase tracking-widest text-on-surface-variant">Customer</label><input id="edit-customer" value={editForm.customer_name} onChange={e => setEditForm(p => ({...p, customer_name: e.target.value}))} className={inputCls} /></div>
                <div><label htmlFor="edit-eta" className="text-xs uppercase tracking-widest text-on-surface-variant">ETA</label><input id="edit-eta" type="date" value={editForm.eta} onChange={e => setEditForm(p => ({...p, eta: e.target.value}))} className={inputCls} /></div>
