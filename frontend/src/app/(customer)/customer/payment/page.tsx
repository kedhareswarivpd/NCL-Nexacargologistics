...
/** Map a backend invoice row onto the shape this page renders. */
function mapInvoice(i: any): Invoice {
  const usdVal = Number(i.total ?? i.amount ?? 0);
  return {
    id: i.invoice_no ?? i.id,
    rawId: i.id,
    shipment: i.shipment_id ?? "—",
    route: i.description ?? "—",
    amount: usdVal * USD_TO_INR,
    amountUsd: usdVal,
        ...
        {stage === "list" && (
          <motion.div key="list" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
            ...
            <div className="grid grid-cols-3 gap-4">
              {[
                { label: "Outstanding",  val: `₹${invoices.filter(i=>i.status!=="Paid").reduce((a,i)=>a+i.amount,0).toLocaleString("en-IN",{minimumFractionDigits:2})}`, color: "text-amber-400" },
                { label: "Overdue",      val: `₹${invoices.filter(i=>i.status==="Overdue").reduce((a,i)=>a+i.amount,0).toLocaleString("en-IN",{minimumFractionDigits:2})}`, color: "text-red-400"   },
                { label: "Paid to Date", val: `₹${invoices.filter(i=>i.status==="Paid").reduce((a,i)=>a+i.amount,0).toLocaleString("en-IN",{minimumFractionDigits:2})}`, color: "text-green-400" },
              ].map(({ label, val, color }) => (
            ...
            {invoices.length === 0 && (
              <Card className="p-10 text-center text-sm text-white/40 border border-white/8 bg-white/[0.02]">
                No invoices to pay yet. They appear here once your shipments are billed.
              </Card>
            )}
            ...
            <Card className="p-5 border border-[#1E88E5]/30 bg-[#0d1e38]/60">
              <div className="flex justify-between items-start gap-4 flex-wrap">
                ...
                <div className="text-right">
                  ...
                  <p className="text-xs text-white/40 mt-0.5">INR</p>
                </div>
              </div>
            </Card>
            ...
            <form noValidate onSubmit={handleSubmit} className="space-y-4">
              ...
              <Card className="p-5 bg-white/[0.02] border border-white/8 space-y-3">
                ...
                {[
                  ...
                  ["Route",         invoice.route],
                  ["Subtotal",      `₹${invoice.amount.toLocaleString("en-IN",{minimumFractionDigits:2})}`],
                  ["Processing Fee","₹0.00"],
                ].map(([k, v]) => (
                ...
                <div className="border-t border-white/8 pt-3 flex justify-between">
                  ...
                  <span className="font-mono font-bold text-[#00C2FF] text-lg">
                    ₹{invoice.amount.toLocaleString("en-IN",{minimumFractionDigits:2})} INR
                  </span>
                </div>
              </Card>
              ...
              <button type="submit"
                ...
                Pay ₹{invoice.amount.toLocaleString("en-IN",{minimumFractionDigits:2})} Securely
              </button>
            </form>
          </motion.div>
        )}
          ...
          <motion.div key="success" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }}
            ...
            <Card className="w-full max-w-md p-6 space-y-4 bg-white/[0.03] border border-white/10 text-left">
              ...
              {[
                ...
                ["Amount Paid",     `₹${invoice.amount.toLocaleString("en-IN",{minimumFractionDigits:2})} INR`, false],
