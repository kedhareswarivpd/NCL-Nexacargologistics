...
export default function InvoicesPage() {
  ...
  async function handleGenerate(e: React.FormEvent) {
    ...
    try {
      ...
      setFormErrors({ amount: apiError(err) });
    }
    setSaving(false);
    load();
  }
  ...
  const filtered = invoices.filter(
    (i) =>
      (i.invoice_no ?? i.id).toLowerCase().includes(search.toLowerCase()) ||
          ...
          {invoices.filter((i) => i.status === "Overdue").map((inv) => (
            <div key={inv.id} className="flex items-center gap-3 rounded-xl border border-error/20 bg-error/5 p-4">
              ...
              <div>
                ...
                <span className="text-sm text-on-surface">{inv.customer_id ?? "—"} — ${inv.amount.toLocaleString()} due {inv.due_date ?? "—"}</span>
              </div>
            </div>
          ))}
        </div>
