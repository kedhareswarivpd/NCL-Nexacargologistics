...
export default function PaymentsPage() {
  ...
  async function load() {
    ...
    try {
      const data = await financeApi.outstanding();
      setOutstanding(data ?? []);
    } catch {
      setOutstanding([]);
    }
  ...
  const filtered = payments.filter(
    (p) =>
      (p.payment_ref ?? p.id).toLowerCase().includes(search.toLowerCase()) ||
          ...
          <tbody className="divide-y divide-white/5">
            {filtered.map((p) => (
              <tr key={p.id} className="hover:bg-white/5 transition-colors">
                <td className="px-4 py-3 font-mono text-xs text-tertiary">{p.payment_ref ?? p.id}</td>
                <td className="px-4 py-3 text-on-surface">{p.customer_id ?? "—"}</td>
                ...
                <td className="px-4 py-3 text-on-surface-variant">{p.paid_at ?? "—"}</td>
